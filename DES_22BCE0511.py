#22BCE0511_Vishnu P K 
#Python Implementation of the Data Encryption Standard (DES) algorithm
...
def bin_to_hex(bin_str):
    return f'{int(bin_str, 2):0{len(bin_str) // 4}X}'
...
def round(input_block, subkey):
    ...
    fprint('P-BOX Permuation', p_box_output)
...
def crypt_string(crypt_type, key_hex, input_hex):
    bin_key = hex_to_bin(key_hex.strip())
    bin_input = pad(hex_to_bin(input_hex.strip()))
    
    subkeys = gen_subkeys(bin_key)
    
    bin_output = ''
    for i in range(0, len(bin_input), BLOCK_SIZE):
        block = bin_input[i:i + BLOCK_SIZE]
        bin_output += des(block, subkeys, crypt_type)

    return bin_to_hex(bin_output)

def main():
    print("DES Encryption/Decryption 22BCE0511)")
    crypt_type = input("Enter operation type (e for encrypt, d for decrypt): ").strip().lower()
    key_hex = input("Enter the key (in hexadecimal): ").strip()
    input_hex = input("Enter the message (in hexadecimal): ").strip()
    result = crypt_string(crypt_type, key_hex, input_hex)
    print(f"\nResult: {result}")
